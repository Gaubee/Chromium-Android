...
public final class DaggerChromeAppComponent implements ChromeAppComponent {
  ...
  private final class BaseCustomTabActivityComponentImpl implements BaseCustomTabActivityComponent {
    ...
    private OriginVerifier.Factory originVerifierFactory() {
    ...
    public WebApkActivityCoordinator resolveWebApkActivityCoordinator() {
      ...
      if (local instanceof MemoizedSentinel) {
        synchronized (local) {
          ...
          if (local instanceof MemoizedSentinel) {
            local = new WebApkActivityCoordinator(ChromeActivityCommonsModule_ProvideChromeActivityFactory.provideChromeActivity(chromeActivityCommonsModule), webappDeferredStartupWithStorageHandler(), webappDisclosureController(), disclosureInfobar(), webApkActivityLifecycleUmaTracker(), ChromeActivityCommonsModule_ProvideLifecycleDispatcherFactory.provideLifecycleDispatcher(chromeActivityCommonsModule), DoubleCheck.lazy(webApkUpdateManagerProvider()));
