...
@IntDef({
    ...
    AccountConsistencyPromoAction.DISMISSED_OTHER, AccountConsistencyPromoAction.AUTH_ERROR_SHOWN,
    AccountConsistencyPromoAction.GENERIC_ERROR_SHOWN,
    AccountConsistencyPromoAction.DISMISSED_BUTTON,
    AccountConsistencyPromoAction.ADD_ACCOUNT_COMPLETED, AccountConsistencyPromoAction.MAX
})
...
public @interface AccountConsistencyPromoAction {
  ...
  int SHOWN = 6;
  /**
   * Promo is not shown due to sign-in being disallowed either by an enterprise policy or by |Allow
   * Chrome sign-in| toggle.
   */
  int SUPPRESSED_SIGNIN_NOT_ALLOWED = 7;
  /**
   * User has added an account and signed in with this account. When this metric is recorded, we
   * won't record SIGNED_IN_WITH_DEFAULT_ACCOUNT or SIGNED_IN_WITH_NON_DEFAULT_ACCOUNT.
   */
